...
5. Nếu yêu cầu một học viên hoặc một lớp KHÔNG thể học được vào một ngày cụ thể (ví dụ: "Phan Duy Minh không học được thứ 2", hoặc "Lớp IELTS-01 bận thứ 5"), hãy tạo ra cấu trúc `class_unavailabilities`. 
   - Bạn cần tìm tất cả các lớp học mà học viên đó tham gia (từ danh sách lớp học ở trên).
   - Với mỗi lớp tìm được, tạo một phần tử có `class_id`, `day` (tiếng Anh viết thường: monday, tuesday, wednesday, thursday, friday, saturday, sunday), và `slots` (mặc định là bận cả ngày: [1, 2, 3, 4, 5]).
6. Nếu yêu cầu có nhắc đến việc thay đổi mức độ ưu tiên (tuyệt đối không, hoặc rất quan trọng), hãy đề xuất `penalties_override` (ví dụ: penalty_paired_classes, penalty_consecutive_limit, penalty_time_preference). Trọng số từ 1 đến 50.
7. Trả về JSON đúng định dạng sau, KHÔNG giải thích lằng nhằng ngoài JSON:
{{
    "paired_class_ids": [["uuid_lop_1", "uuid_lop_2"]],
    "class_preferences": [{{"class_id": "uuid_lop", "preferred_time_period": "morning"}}],
    "class_unavailabilities": [{{"class_id": "uuid_lop", "day": "monday", "slots": [1, 2, 3, 4, 5]}}],
...
"""
        ...
        try:
            ...
            return AIAnalyzeResponse(
                ...
                class_unavailabilities=result_json.get("class_unavailabilities", []),
